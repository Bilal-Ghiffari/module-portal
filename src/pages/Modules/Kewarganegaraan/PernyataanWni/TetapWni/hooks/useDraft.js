...
export const useDraft = (
  ...
  const handleDraft = useCallback(
    async (activeStep, formik, setStepErrors) => {
      ...
      // Filter formik to find if not id_kewarganegaraan & status_kawin
      const excludeField = [
      ...
      // Filter formik to find if not id_kewarganegaraan & status_kawin
      const filteredRawCurrent = Object.fromEntries(
        Object.entries(rawCurrentStepValues).filter(
          ([key]) => !excludeField.includes(key)
        )
      );

      console.log(
        Object.keys(getFilledValues(filteredRawCurrent, currentStepFields))
      );
