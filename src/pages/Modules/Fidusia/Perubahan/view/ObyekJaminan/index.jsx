...
import ObyekJaminanTable from './components/ObjectJaminanTable';
...
const ObyekJaminan = ({
  ...
  const [totalNominalRupiah, setTotalNominalRupiah] = useState(0);

  const [tableData, setTableData] = useState([]);

  const objectJaminan = formik.values.object_jaminan;
  // console.log('formik obuekk', formik);

  const onDataChange = useCallback((data) => {
    onUpdateData(data);
    setTableData(data.object_jaminan || []);
  }, []);
  ...
  useEffect(() => {
    setTableData(dataSource.object_jaminan || []);
  }, [dataSource]);
  ...
  return (
    <>
      ...
      <LineDashed />
      <ObyekJaminanTable objectJaminan={tableData} />
      ...
      <NilaiPenjaminanSection
        ...
        totalNominalRupiah={totalNominalRupiah}
      />
      <LineDashed />
      ...
      <AgreementSection
        formik={formik}
        aggrement={aggrement}
        onAggrementChange={onAggrementChange}
      />
    </>
  );
};
