...
const FormIdentitasPemberi = ({ formik, disabled = false }) => {
  ...
  const onDataChange = useCallback((data) => {
    ...
    formik.setFieldValue('identity_pemberi', {
      ...data.identity_pemberi,
      jenis_pendaftaran: data.identity_pemberi.jenis_pendaftar,
    });
  }, []);
  // console.log('formi identity>>', identityPemberi);
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      try {
        await onFetchLoadFidusiaPendaftaran(); // fetch the data when component mounts
      } catch (error) {
        console.error('Error fetching initial data:', error);
        toastifyService.customWarningMsg('Error fetching data');
      }
    };
  ...
  if (loading || !identityPemberi)
    return <AHULoading open={loading} message="Memuat Data" />;
