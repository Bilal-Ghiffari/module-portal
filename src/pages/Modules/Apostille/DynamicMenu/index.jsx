...
import {
  apiGetListApostillePermohonanRiwayat,
  apiGetListApostilleVerifikasiPermohonanRiwayat,
...
const DynamicMenu = ({ label = "" }) => {
  ...
  const fetchData = (query, label) => {
    toastifyService.showLoading();
    switch (label) {
      case "Daftar Verifikasi":
        apiGetListApostilleVerifikasiPermohonanRiwayat(query)
          .then((res) => {
            setTableData({
              data: res.data,
              total_count: res.pagination.totalData,
            });
          })
          .catch((err) => {
            console.log("err", err);
